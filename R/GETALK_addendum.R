...
catch_by_month_and_bormicon <- function(dat,gear,Land.gear,area=c(1:10),veman=c(1:12)) {
  
  tmp <- matrix(0,12,10)
  dimnames(tmp) <- list(1:12,1:10)
  
  i <- dat$area %in% area & dat$veman %in% veman & dat$veidarf %in% gear
...
#' @export
#' 
#' @param ar Numerical. Year
#' @param teg Numerical. Species code
#' @param veidarfaeri Numerical vector. Containing gear to include.
#' @param synaflokkur Numerical vector. Containing sampleclass to use.
read_pax_data <- function(ar,teg,veidarfaeri,synaflokkur) {
  stodvar <- lesa.stodvar(ar = ar, veidarfaeri = veidarfaeri)
  ...
  stodvar <- stodvar[stodvar$area %in% c(1:13,15),]
  stodvar <- calcgear(stodvar)
  stodvar$gear <- stodvar$vf # Copy of column
  stodvar$reg <- rep("s2",nrow(stodvar)) 
  i <- !is.na(match(stodvar$area,c(1,9,10,13)))
  stodvar$reg[i] <- "s1"
  stodvar$time <- rep("t1",nrow(stodvar))
  i <- stodvar$man > 5
  stodvar$time[i] <- "t2"
  stodvar$index <- paste(stodvar$vf,stodvar$reg,stodvar$time,sep="")
  
  kvarnir <- lesa.kvarnir(stodvar$synis.id, teg, c("kyn", "kynthroski","slaegt","oslaegt"))
  ...
  lengdir <- lesa.lengdir(lengdir_synis.id, teg)
